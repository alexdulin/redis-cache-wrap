...
class Cache {
  constructor (opts) {
    ...
    this.conn = opts.client || this.createClient(opts.connection);
  }
   ...
   *
   */
  async _wrapPromise (key, ex, worker) {
    const self = this;

    if (this.isFn(ex) && !worker) {
      worker = ex;
      ex = self.ex;
    }

    if (!key || typeof key !== 'string') {
      return Error('Invalid \'key\' passed to cache. \'key\' must be a string.');
    }
  ...
  wrap (key, ex, worker, done) {
    if (this.isFn(ex) && !done) {
      done = worker;
      worker = ex;
      ex = this.ex;
    }

    return this._wrapPromise(key, ex, worker).asCallback(done);
  }
  ...
  wrapPromise (key, ex, worker) {
    return this._wrapPromise(key, ex, worker);
  }
}
