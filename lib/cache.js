...
redis.RedisClient.prototype = Promise.promisifyAll(redis.RedisClient.prototype);
redis.Multi.prototype = Promise.promisifyAll(redis.Multi.prototype);
 ...
 */
Cache.prototype._do = function (...args) {
 ...
 * @return {Promise}
 */
Cache.prototype.keys = function keys (pattern = '') {
  if (typeof pattern !== 'string' || !pattern.length) {
    pattern = this.prefix + '*';
  }

  return this.conn.keysAsync(pattern);
};
 ...
 *
 */
Cache.prototype.flush = function (pattern) {
  const self = this;
  ...
  return self.conn.keysAsync(pattern).then(keys => {
    if (Array.isArray(keys) && keys.length) {
      return self.conn.delAsync(keys);
    }
    return null;
  });
};
...
Cache.prototype.set = function (key, value, ex, cb) {
  ...
  if (isFn(cb)) {
    ...
    return this.conn.setAsync(key, value, 'EX', ex);
  }
};
