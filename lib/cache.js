...
Cache.prototype.flush = function (pattern) {
  ...
  return self.conn.keys(pattern).then(keys => {
    if (Array.isArray(keys) && keys.length) {
      return self.conn.del(keys);
    }
    return null;
  });
};
...
const wrapPromise = bluebird.coroutine(function * (context, key, ex, worker) {
  ...
  if (!key || typeof key !== 'string') {
    return bluebird.reject(Error('Invalid \'key\' passed to cache. \'key\' must be a string.'));
  }

  try {
    let res = yield self.get(key);
    if (res) {
      return bluebird.resolve(JSON.parse(res));
    }
